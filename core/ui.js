...
async function promptUserPreferences(previous = {}) {
    // Use toggle-style prompts for better UX
    const { enablePositionLogging } = await prompt({
        type: 'select',
        name: 'enablePositionLogging',
        message: chalk.bold('Position logging:'),
        choices: [
            { title: chalk.green('✓ ON'), value: true },
            { title: chalk.red('✗ OFF'), value: false }
        ],
        initial: previous.enablePositionLogging !== undefined ? (previous.enablePositionLogging ? 0 : 1) : 0
    });
    ...
    const { defaultTimeframe } = await prompt({
        ...
        message: chalk.cyan('Default timeframe:'),
        choices: [
            { title: '1 Minute', value: '1Min' },
            { title: '5 Minutes', value: '5Min' },
            { title: '15 Minutes', value: '15Min' },
            { title: '1 Hour', value: '1Hour' },
            { title: '1 Day', value: '1Day' }
        ],
        initial: ['1Min', '5Min', '15Min', '1Hour', '1Day'].indexOf(previous.defaultTimeframe || '5Min')
    });
    
    const { enableCrossunderSignals } = await prompt({
        type: 'select',
        name: 'enableCrossunderSignals',
        message: chalk.bold('MA crossunder signals for selling:'),
        choices: [
            { title: chalk.green('✓ ON'), value: true },
            { title: chalk.red('✗ OFF'), value: false }
        ],
        initial: previous.enableCrossunderSignals !== undefined ? (previous.enableCrossunderSignals ? 0 : 1) : 0
    });
    
    const { enablePerformanceMetrics } = await prompt({
        type: 'select',
        name: 'enablePerformanceMetrics',
        message: chalk.bold('Advanced performance metrics:'),
        choices: [
            { title: chalk.green('✓ ON'), value: true },
            { title: chalk.red('✗ OFF'), value: false }
        ],
        initial: previous.enablePerformanceMetrics !== undefined ? (previous.enablePerformanceMetrics ? 0 : 1) : 1
    });
